...
RECORD_HZ = 10
GRIPPER_DEBOUNCE_S = 0.2   # seconds between accepted A/B presses
GRIPPER_FUNCS = {
    "MoveGripper", "ActGripper", "GetGripperCurPosition",
    "SetGripperConfig", "GetGripperConfig", "GetGripperMotionDone",
    "GetGripperActivateStatus", "GetGripperCurCurrent",
    "GetGripperCurSpeed", "GetGripperVoltage", "GetGripperTemp",
}
...
class RobotController:
    def __init__(self):
        ...
        self.gripper_pos = 100  # 0=closed, 100=open
        self._last_speed_adj = 0.0
        self._gripper_busy = threading.Event()   # set = a gripper command is in flight
        self._last_a_press_t = 0.0
        self._last_b_press_t = 0.0
    ...
    def _call(self, func, *args):
        lock = self._gripper_lock if func in GRIPPER_FUNCS else self._arm_lock
        with lock:
            try:
    ...
    def _stop_gripper(self):
        self._gripper_busy.set()
        try:
            ret = self._call("GetGripperCurPosition")
            print(f"  [DBG] GetGripperCurPosition raw: {ret}")
            try:
                pos = int(ret[2])
            except (TypeError, IndexError):
                pos = self.gripper_pos
            self.gripper_pos = pos
            print(f"Gripper stop at {pos}%")
            self._call("MoveGripper", 1, pos, 50, 50, 30000, 0, 0, 0, 0, 0)
        finally:
            self._gripper_busy.clear()
    ...
    def _print_controls(self):
        ...
        print("  LT = Jaw-  |  RT = Jaw+")
        print("  A (hold) = Open Gripper  |  B (hold) = Close Gripper  |  D-Pad Down = Stop Gripper")
            ...
            try:
                ...
                for event in events:
                    ...
                    # D-Pad
                        ...
                        if not self._gripper_busy.is_set():
                            threading.Thread(target=self._stop_gripper, daemon=True).start()
                    ...
                    # A / B press starts motion, release stops at current position
                    elif event.code == "BTN_SOUTH":          # A
                        now = time.time()
                        if event.state == 1:
                            if self._gripper_busy.is_set():
                                continue
                            if now - self._last_a_press_t < GRIPPER_DEBOUNCE_S:
                                continue
                            self._last_a_press_t = now
                            threading.Thread(target=self._open_gripper, daemon=True).start()
                        else:
                            if not self._gripper_busy.is_set():
                                threading.Thread(target=self._stop_gripper, daemon=True).start()
                        ...
                        if event.state == 1:
                            if self._gripper_busy.is_set():
                                continue
                            if now - self._last_b_press_t < GRIPPER_DEBOUNCE_S:
                                continue
                            self._last_b_press_t = now
                            threading.Thread(target=self._close_gripper, daemon=True).start()
                        else:
                            if not self._gripper_busy.is_set():
                                threading.Thread(target=self._stop_gripper, daemon=True).start()
