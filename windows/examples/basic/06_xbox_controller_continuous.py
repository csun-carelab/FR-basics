"""
06_xbox_controller_continuous.py — Xbox controller teleoperation for FR5.
A (hold) = Open gripper +5% | B (hold) = Close gripper -5%
...
USER_NUM        = 0
GRIPPER_STEP    = 5    # percent per main loop tick while button held
...
def gripper_open_step():
    global gripper_pos
    gripper_pos = min(100, gripper_pos + GRIPPER_STEP)
    print(f"Gripper: {gripper_pos}%")
    # SDK: MoveGripper(index, pos, vel, force, maxtime, block, type, rotNum, rotVel, rotTorque)
    call("MoveGripper", 1, gripper_pos, 100, 50, 5000, 0, 0, 0, 0, 0)

def gripper_close_step():
    global gripper_pos
    gripper_pos = max(0, gripper_pos - GRIPPER_STEP)
    print(f"Gripper: {gripper_pos}%")
    call("MoveGripper", 1, gripper_pos, 100, 50, 5000, 0, 0, 0, 0, 0)
...
def print_controls():
    ...
    print("  LT = Yaw-  |  RT = Yaw+")
    print("  A (hold) = Open Gripper +5%  |  B (hold) = Close Gripper -5%")
...
def process_event(event):
    global mode, speed, sel_joint, lb_held, rb_held, a_held, b_held, home_req
    ...
    if event.code == "BTN_TL":                                  # LB
        ...
        if event.state == 1 and mode == "JOINT":
            ...
            print(f"Joint: J{sel_joint}")
    elif event.code == "BTN_SOUTH":                             # A = open step (hold)
        a_held = (event.state == 1)
    elif event.code == "BTN_EAST":                              # B = close step (hold)
        b_held = (event.state == 1)
    elif event.state == 1:
